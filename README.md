...
## Extra notes
In the `results` folder there are 5 subfolders, each corresponding to one training step of the algorithm. Note that some training requires an extension, a `second_training`, which continues training on the final checkpoint of the first training. Some results may have been obtained before the standardization of file/folder names in the repository, so all the elements pertinent to each training are archived with their original names in the corresponding folder. 
