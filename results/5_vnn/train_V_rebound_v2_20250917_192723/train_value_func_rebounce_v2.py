...
PLEASE READ 4_Training_ValueFunction_global.md for detailed explanation
"""
...
# ---- Parsers ----------- #
    ...
    parser.add_argument('-t', '--config_train', type=str, required=True, help='path to the config files with hyper parameters for this training')
    ...
    parser.add_argument('--opt-args', type=str, help='Arguments for the optimizer in the format key1:value1,key2:value2.... If passed, they replaces the ones in the config file if present. See the pytorch docs for the parameters available for each optimizer.')
        ...
        # for the interior
    parser.add_argument('-a', '--a_config', type=str, required=True, help='path to the parameters of the trained network a_nn (augmented optimal control)')
        
        # for the boundary
    parser.add_argument('-v', '--V_bound_config', type=str, required=True, help='path to the parameters of the trained netwrok V_bound (value function at the boundary)')
    parser.add_argument('-u', '--u_config', type=str, required=True, help='path to the parameters of the trained network u_nn (optimal control at the boundary)')
    parser.add_argument('-w', '--w_config', type=str, required=True, help='path to the parameters of the trained network w_nn (boundary value function)')
    ...
    results_dir = utils.create_dir(basedir=args.results_dir, dirname = "train_V_rebound_v2", suffix = None)
    logger = utils.setup_logging(results_dir, log_level = args.log_level, fname='train_V_rebound_v2.log')
    ...
    shutil.copy2(args.config_train, J(results_dir,'config_train_V.json'))
    ...
    a_dir = args.a_config
    with open(J(a_dir, "config_train_a.json"), 'r') as f:
    ...
    # Load V_bound_nn
        # load the configuration
    V_bound_dir = args.V_bound_config
    config_V_bound_nn = json.load(open(J(V_bound_dir, "config_train_V_bound.json"), 'r')) # config

        # architecture of V_bound_nn
    # TODO: there is no args.json file in all of V_bound training folders, so we assume it's 'residual-sin'. Will fix this to look like w_NN in the final script
    V_bound_NN = custom_nn.PortfolioValueNet(num_time_freqs=8, state_dim=dim_state, hidden_dim=config_V_bound_nn['n_neuron'], 
                                num_layers=config_V_bound_nn['n_hidden_layers'], time_encoding='sinusoidal', norm_type='identity')
...
# ---- Directory --------- #
    ...
    json.dump(config_V_bound_nn, open(f'{results_dir}/config_V_bound.json', 'w'), indent=4)

    json.dump(config, open(f'{results_dir}/config_train_V.json', 'w'), indent=4)
    args_dict = vars(args)
    args_dict['config_unn'] = os.path.abspath(args.u_config)
    args_dict['config_wnn'] = os.path.abspath(args.w_config)
    args_dict['config_ann'] = os.path.abspath(args.a_config)
    args_dict['config_V_bound_nn'] = os.path.abspath(args.V_bound_config)
