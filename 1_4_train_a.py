...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '--config_a', type=str, required=True, help='path to the configuration file with training parameters for augmented optimal control neural network')
    parser.add_argument('-w', '--config_w', type=str, required=True, help='path to the results folder of the domain boundary value network')
    ...
    results_dir = utils.create_dir(basedir=args.results_dir, dirname = "train_a", suffix = None)
    logger = utils.setup_logging(results_dir, log_level = args.log_level, fname='train_a.log')
    ...
    if args_wnn['arch'] == 'mlp':
        ...
        w_NN = custom_nn.BoundaryValueNetwork(dim_input = dim_state + 1,
                                ...
                                n_hidden = config_w['n_hidden_layers'],
                                activation = torch.nn.ReLU())
    elif args_wnn['arch'] == 'residual':

        w_NN = custom_nn.PortfolioValueNet(num_time_freqs=8, state_dim=dim_state, hidden_dim=config_w['n_neuron'], num_layers=config_w['n_hidden_layers'], time_encoding='none', norm_type=args_wnn['norm_type'])

    elif args_wnn['arch'] == 'residual-sin':

        w_NN = custom_nn.PortfolioValueNet(num_time_freqs=8, state_dim=dim_state, hidden_dim=config_w['n_neuron'], num_layers=config_w['n_hidden_layers'], time_encoding='sinusoidal', norm_type=args_wnn['norm_type'])

    elif args_wnn['arch'] == 'residual-learnable':

        w_NN = custom_nn.PortfolioValueNet(num_time_freqs=8, state_dim=dim_state, hidden_dim=config_w['n_neuron'], num_layers=config_w['n_hidden_layers'], time_encoding='learnable', norm_type=args_wnn['norm_type'])
    ...
    shutil.copy(J(u_dir, "config_u.json"), J(results_dir,'config_u.json'))
    ...
    config = json.load(open(args.config_a, 'r'))
    ...
    json.dump(config, open(f'{results_dir}/config_a.json', 'w'), indent=4)
    ...
    for epoch in range(start_epoch, n_epoch):
        ...
        # progress tracking
        id_path_full = taus == T
        epoch_n_paths_full = id_path_full.sum()
