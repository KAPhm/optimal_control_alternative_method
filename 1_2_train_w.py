...
# ---- Parser ------- # 
    ...
    parser.add_argument('-w', '--config_w', type=str, required=True, help='path to the configuration file with all the parameters for the training of the value network (w)')
    ...
    config = json.load(open(args.config_w, 'r'))  # retrieve configuration for training
...
# ---- Directory ---- #
    ...
    results_dir = utils.create_dir(basedir=args.results_dir, dirname = "train_w", suffix = None)
    checkpoint_dir = utils.create_dir(basedir=results_dir, dirname = "checkpoints", suffix = "")
    logger = utils.setup_logging(results_dir, log_level = args.log_level, fname='train_w.log')
    ...
    json.dump(config, open(f'{results_dir}/config_w.json', 'w'), indent=4)
